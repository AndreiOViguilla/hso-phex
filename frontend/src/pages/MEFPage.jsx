...
export default function NurseMEFPage({ studentMongoId, onBack, onSaved }) {
  ...
  useEffect(() => {
    ...
    console.log("[NurseMEF] sync effect fired, map size:", Object.keys(map || {}).length);
  ...
  const renderPreview = useCallback(async () => {
    ...
    try {
      ...
      console.log("[NurseMEF] annotationDiv:", !!annotationDiv, "pdfjsViewer:", !!window.pdfjsViewer);
      if (annotationDiv && window.pdfjsViewer) {
        ...
        try {
          ...
          // DEBUG: inspect what pdf.js actually puts on widgets
          console.log("[NurseMEF] field map keys:", Object.keys(map));
          const sample = annotationDiv.querySelector("input, textarea, select");
          if (sample) {
            console.log("[NurseMEF] sample widget attrs:", {
              name: sample.name,
              id: sample.id,
              dataElementId: sample.getAttribute("data-element-id"),
              outerHTML: sample.outerHTML.slice(0, 200),
            });
          }
        } catch (err) {
          console.error("[NurseMEF] annotation render error:", err);
        }
      }
      if (tooltipDiv) {
